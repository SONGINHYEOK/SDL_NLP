...
# 시약/재고/장비 유지보수 데이터 (시약 12종, 재고 24 lot, 유지보수 7건)
python manage.py runscript seed_inventory
...
| Maintenance | `/equipment/maintenance/` | 장비 캘리브레이션/정비 일정, 기한 초과 알림 |
| Inventory | `/inventory/` | 시약 12종 재고 추적, FIFO 소모, 부족 알림 |
...
Design → Synthesize → Formulate → Analyze → Learn
  │          │            │           │          │
  │          │            │           │          └─ AI 모델 성능 개선
  │          │            │           └─ Experiment + ExperimentResult 생성 + 시약 소모 (Cuvette, Plate)
  │          │            └─ LNPFormulation 레코드 생성 + 시약 소모 (Buffer, mRNA, Cartridge)
  │          └─ 합성 조건 시뮬레이션 + 장비 로그 + 시약 소모 (Ethanol, DCM)
...
| Synthesize | EquipmentStatus x 2, ReagentConsumption x 2 | yield, purity, synthesis_conditions, reagents_consumed |
| Formulate | LNPFormulation x 1, EquipmentStatus x 2, ReagentConsumption x 4 | formulation_pk, composition, N/P ratio, reagents_consumed |
| Analyze | Experiment x 1, ExperimentResult x 3, EquipmentStatus x 2, ReagentConsumption x 2 | diameter, PDI, zeta, EE%, luminescence, reagents_consumed |
...
├── equipment/               # 장비 모니터링 + 유지보수
│   └── models.py            #   Equipment(5), EquipmentStatus, MaintenanceRecord
├── inventory/               # 시약/소모품 재고 관리
│   └── models.py            #   Reagent(12), ReagentStock(24), ReagentConsumption
...
│   └── seed_inventory.py    # 시약 12종 + 재고 24 lot + 유지보수 7건
├── data/seed/
...
5. Inventory → 시약 재고 현황 확인, 부족 알림
6. Maintenance → 장비 캘리브레이션/정비 이력
7. Workflow → 새 워크플로우 생성 → 5단계 순차 실행 → AI 보고서 (시약 자동 차감 확인)
