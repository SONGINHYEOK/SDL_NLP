...
/compounds/compare/            -> compounds:compare       [SKELETON]
/compounds/helpers/            -> compounds:helpers        [SKELETON]
...
/experiments/                  -> experiments:list        [SKELETON -> P0]
/ai/predict/                   -> ai_models:predict       [SKELETON -> P0]
...
```
Phase 1 - Core Demo Loop (P0)
  1. experiments/list.html        <- View 로직 있음, 템플릿만 구현
  2. equipment/monitor.html       <- View + DRF API 있음, 템플릿만 구현
  3. workflow/pipeline.html       <- View + Model 있음, 템플릿만 구현
  4. workflow/run_detail.html     <- View 있음, 템플릿만 구현
  5. ai_models/predict.html       <- View + API 있음, 템플릿만 구현

Phase 2 - AI Features (P1)
  6. ai_models/generate.html      <- View 있음, 템플릿 구현
  7. ai_models/optimize.html      <- View 있음, 삼각 다이어그램 + Pareto front
  8. compounds/compare.html       <- 신규 View + 템플릿

Phase 3 - Polish (P2)
  9. Dashboard 고도화              <- 위젯 추가
  10. 로그인/인증                   <- accounts app 활용
```
...
| 생성형 AI 기반 이온화지질 구조 설계 프로토타입 | AI Generate 페이지 | TODO (P1) |
| LNP 제형 후보 설계 AI 모델 프로토타입 | AI Predict + Formulation Designer | DONE |
| 생성형 AI 기반 실험 조건 제안 알고리즘 초기 모델 | AI Optimize 페이지 | TODO (P1) |
| AI-자동화 장비 간 통신 인터페이스 설계 | Equipment REST API + Monitor | API DONE, UI TODO (P0) |
| 자동화 장비-로봇-AI 자율실험실 시스템 구현 | Workflow Pipeline | TODO (P0) |
| 웹 기반 플랫폼 프로토타입 개발 | 전체 플랫폼 | 60% 완료 |
